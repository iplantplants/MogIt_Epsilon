...
--epsilon edit
mog.frame:SetPropagateKeyboardInput(true) --epsi edit
mog.frame:SetScript("OnKeyDown", function(self, key)
...
mog.frame.portrait:SetTexture("Interface\\AddOns\\MogIt\\Images\\MogIt_Epsilon");
...
--// Catalogue Menu
toggle = 0;
function toggleSheathe(self)
	--print(toggle)
	if toggle == 0 then
		for i, model in ipairs(mog.models) do
			model.model:SetSheathed(true)
		end
		toggle = 1;
	elseif toggle == 1 then
		for i, model in ipairs(mog.models) do
			model.model:SetSheathed(false)
		end
		toggle = 0;
	end
	--print(toggle)
...
function epsilon_CreateGenderMenu(dropdown, level, func, selectedGender)
	for i = 0, 1 do
		local info = UIDropDownMenu_CreateInfo();
		info.text = gender[i];
		info.func = func;
...
mog.menu.catalogue = mog.menu:CreateMenu(L["Catalogue"], function(self, tier)
	if tier == 1 then
		...
		local info = UIDropDownMenu_CreateInfo();
		info.text = L["|cff00ccffWeapon Sheathe|r"];
		info.value = "weaponSheathe";
...
help:SetScript("OnEnter", function(self)
	...
	GameTooltip:AddLine(L["|cff00ccffPress "..GetBindingKey("TOGGLESHEATH").." to toggle weapon sheathe|r"], 1, 1, 1);
