...
mog.view:SetScript("OnShow",function(self)
	sheatheKey = GetBindingKey("TOGGLESHEATH");
...
mog.view:SetPropagateKeyboardInput(true)

mog.view:SetScript("OnKeyDown", function(self, key)
	if key ~= sheatheKey then --epsilon sheathe
		--do nothing
	else
		--self:SetPropagateKeyboardInput(key~="Z")
		--print(key.." is sheathe toggle")
		toggleSheath();
		return true
	end
	--print(key)
...
local function toggleSheathe(self)
	if toggle == 0 then
		MogItPreview1.model.model:SetSheathed(true)
		toggle = 1;
	elseif toggle == 1 then
...
local function setNPCRace(self,_,raceID)
	--print(raceID);
	SendChatMessage(".ph forge npc outfit race "..raceID)
end



function setNPCGender(self,raceID,genderID)
	...
	SendChatMessage(".ph forge npc outfit gender "..genderString)
end
...
local function helpOnEnter(self)
	...
	GameTooltip:AddLine(L["|cff00ccffPress "..sheatheKey.." to toggle weapon sheathe|r"],1,1,1);
	GameTooltip:AddLine(" ");
